...
function endpoint_kibua(){
   wp_localize_script('jquery','kibua',array(
     'ajax' => admin_url('admin-ajax.php')
   ));
}


add_action('wp_enqueue_scripts', 'endpoint_kibua');


add_shortcode('kibua_form', function () { // insurance form shortcode
  ob_start();
  kibua_form_html();
  return ob_get_clean();
});
 ...
 <label id="validtext" align="right">נא מלא את כל השדות</label>
  ...
  <div class="step" display="block">
    ...
    <input  id="fname" type="text" placeholder="הכנס את שימך" name="fname">
     ...
     <label  class="koteret" align="right">מהו סטטוס התעסוקה שלך?</label>
    
        <div  class="blocks"> 
    
    <label><input type="radio" name="employ" class="item-list" value="עד 50"  ><span>יצאתי לפנסיה</span></label>
  <label><input type="radio" name="employ" class="item-list" value="51-65" ><span>עדיין עובד</span></label>
...
למשל: תיקי השקעות, קרנות השתלמות, קופות גמל, עו"ש, פוליסת חיסכון, פק"מ ועוד
(מידע זה ישמש לצורך בדיקת זכאות בלבד)</label>
   <div  class="blocks"> 
...
<label><input type="radio" name="savings"  value="300-500K"  ><span>300 ל- 500 אלף ₪</span></label>
  ...
  <div class="step" style="display: none;">
    ...
    <label class="koteret" align="right">זכאותך לקיבוע זכויות ותכנון פרישה תלויה בין היתר בגובה הקצבה הנוכחית\עתידית שלך.
האם קצבת הפנסיה הנוכחית\עתידית שלך מעל 4,500 ש"ח?
 ...
 const regex = /^05([0-9])[0-9]{7}$/;
  ...
  debugger
      ...
      function sendDataCrm(){
       ...
       const employ = jQuery('input[name="employ"]:checked').val();
      ...
      jQuery.ajax({
       ...
       url: kibua.ajax,
        ...
        /// location.replace('https://www.tiktax.co.il/?page_id=191');
          else window.location = "https://nownet.co.il/?page_id=488";
        },
      })
      }
...
send_sms_kibua();
 ...
 //   'age' =>  $_POST['age'], //status_partner
    'pens_above' =>  $_POST['pens_above'], //salary
    'saving' =>  $_POST['savings'], //salary_partner
 'address1' =>  $_POST['address1'], //salary_partner
   'fld_198234' =>  $_POST['fld_198234']
     ];
...
add_action('wp_ajax_send_data_crm4','send_data_crm4');
add_action('wp_ajax_nopriv_send_data_crm4','send_data_crm4');


function send_sms_kibua(){
...
function check_code_kibua(){
 $code_user = $_POST['code_input'];
 $codeSend =  $_SESSION['code'];
 $current_code =  $codeSend === $code_user;
 if($current_code) success_sms_kibua();
 echo $current_code;  
  wp_die();
}
add_action('wp_ajax_check_code_kibua','check_code_kibua');
add_action('wp_ajax_nopriv_check_code_kibua','check_code_kibua');
  

function success_sms_kibua(){
